...
# Cell QC
min_total_counts <- 750
max_total_counts <- 10000
min_total_genes <- 200
max_total_genes <- 3000
...
# Introduction
We are investigating how intratumorigenic heterogeneity of chronic lymphocytic leukemia (CLL) evolves during its clincial course, with a particular emphasis on Richter transformation. The objective of this notebooks is to filter out poor-quality cells and genes and normalize the expression counts.   
...
```{r}
donor <- str_remove(colnames(richter), "ICGC_")
donor <- str_remove(donor, "_.*")
table(donor)
richter$donor <- donor
ggplot(richter@meta.data, aes(donor, log(nCount_RNA), fill = donor)) +
  geom_boxplot() +
...
We can see that the donor "019" has fewer counts than the other two donors ("012" and "365"). Nevertheless, the distributions overlap and, therefore, we will not perform QC separately.
...
we see a bimodal distribution, with the first peak representing poor-quality cells. We will retain cells that have at least `r min_total_counts_1220` counts. Moreover, we require less than `r max_total_counts_1220`
...
* Number of detected genes: < `r min_total_genes`, > `r max_total_genes` max_total_genes_no_1220`
...
richter@meta.data %>% 
  ...
  ggplot(aes(donor, pct_low_quality, fill = donor)) +
    geom_col() + 
    labs(x = "% low-quality cells") +
    theme_classic() +
    theme(legend.position = "none")
```
...
```{r}
row_data_frame <- data.frame(symbol = rownames(richter_sce))
rownames(row_data_frame) <- rownames(richter_sce)
rowData(richter_sce) <- row_data_frame
richter_sce <- calculateQCMetrics(richter_sce)
plotHighestExprs(richter_sce, feature_names_to_plot = "symbol")
```
...
Note: where is the call to normalize? 
